...
class Taxi:
    ...
    def __init__(self, world, taxi_num, idle_loss=256, max_wait=50, on_duty_time=0, off_duty_time=0, service_area=None,
        ...
        #map = {node: (neighbour, distance)}, neighbour = (x,y) distance = (x,y)
        ...
        for fare in self._availableFares.items():
            ...
            if len(self._path) == 0 and fare[1].allocated and self._passenger is None:
                ...
                if self._loc.index[0] == origin[0] and self._loc.index[1] == origin[1]:
                    ...
                    if self._passenger is not None:
                        #self._aStarPath(self._loc.index, self._passenger.destination)
                        self._path = self._planPath(self._loc.index, self._passenger.destination)
    ...
    def drive(self, newPose):
        #Reminder: newPose==(Node node, dir 0-7)
    ...
    def recvMsg(self, msg, **args):
        ...
        # we just dropped off a fare and received payment, add it to the account
            ...
            print(f"Taxi{self.number}: Before: ({self._account})")
            self._account += args['amount']
            print(f"Taxi{self.number}: After: ({self._account})")
            return
    ...
    # to do much better than this! -> Not anymore!
    def _notplanPath(self, origin, destination, **args):
                ...
                if destination in path:
                    ...
                    if len(path) > 1:
                        try:
                            ...
                            #Changed pnode -> badNode, educated guess given pnode not defined at IndexError loc
                            raise IndexError("Invalid path: no route from ({0},{1}) to ({2},{3} in map".format(
                                self._map[path.index(badNode) - 1][0], self._map[path.index(badNode) - 1][1], badNode[0],
                                badNode[1]))
    ...
    #Utilises an 'as crows fly' heuristic between nodes to guesstimate the most efficient path to take
    def _planPath(self, origin, destination, **args):
        if origin not in self._map or destination not in self._map:
            print (f"planPath: Origin ({origin}) or destination ({destination}) not in world {self._world.locations}.")

        if origin == destination:
            print("plantPath: Already at destination")
            return []

        startNode = self._world.getNode(origin[0], origin[1])
        goalNode = self._world.getNode(destination[0], destination[1])

        open_set = []

        #child->parent == (x,y):(x,y)
        from_to = {}
        #Index by (x,y) rather than node object to reduce size of data structure
        gScore = {origin: 0}
        fScore = {origin: self._world.distance2Node(startNode, goalNode)}

        #BUGFIX: When fScore is equal between nodes heapq moves to the next value in the tuple to compare, which
        #        causes a TypeError; I added counter to prevent heappop from comparing node objects.
        counter = 0


        heapq.heappush(open_set, (fScore[origin], counter, startNode))
        while len(open_set) > 0:
            # _ is throwaway for fScore of node and counter as numpy
            ...
            if current.index == destination:
                ...
                #Take steps through path
                while step in from_to:
                    path.append(step)
                    step = from_to[step]
                #
                path.append(origin)
                ...
                #print (f"ASTAR PATH FROM {origin} to {destination}:\n", path)
            ...
            for direction, x, y in neighbours:
                neighIdx = (x,y)
                neighbour = self._world.getNode(x, y)
                newG = gScore[current.index] + self._world.distance2Node(current, neighbour)

                #If this node doesn't have a gscore or we've found a better one
                if neighIdx not in gScore or gScore[neighIdx]>newG:
                    #Update the gScore
                    from_to[neighIdx] = current.index
                    gScore[neighIdx] = newG
                    fScore[neighIdx] = newG + self._world.distance2Node(neighbour, goalNode)

                    if neighbour not in open_set:
                        heapq.heappush(open_set, (fScore[neighIdx], counter, neighbour))
                        counter+=1

        print("astar couldn't find path")
        return []
