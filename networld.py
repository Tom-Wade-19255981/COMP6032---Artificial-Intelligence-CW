...
class NetWorld:
   ...
   '''
                    ...
                    for edge in edges:
                        ...
                        if edge.dirA < 2 or edge.dirA > 6:
                            if src.index[1] <= dst.index[1]:
                                raise ValueError(
                                    "Road exit point from node {0} points away from destination node {1}".format(
                                        src.index, dst.index))
                            srcExity -= 1
                        ...
                        if nextNodeIdx[0] == dst.index[0] and nextNodeIdx[1] == dst.index[1]:
                            if self.addEdgeSegment(src, nextNodeIdx, edge.bidirectional) != dst:
                                raise KeyError(
                                    "Indexed node ({0},{1}) should be the end junction {2}".format(nextNodeIdx[0],
                                                                                                   nextNodeIdx[1],
                                                                                                   dst.index))
                            return
                        ...
                        if not edge.bidirectional:
                            ...
                            if self.addEdgeSegment(penultimateNode, (dst.index[0], dst.index[1]), False) != dst:
                                raise KeyError("Penultimate node {0} should be adjacent to end junction {1}".format(
    ...
    def allocateFare(self, origin, taxi):
        ...
        if taxi.number in self.allocated_fares:
            self.allocated_fares[taxi.number] += 1
        else:
            self.allocated_fares[taxi.number] = 1

        taxi.recvMsg(taxi.FARE_ALLOC, **{'origin': origin, 'destination': self._fareQ[origin].destination})
        print("networld: Fare {0} allocated to T{1}".format(origin, taxi.number))
    ...
    def runWorld(self, ticks=0, outputs=None, outlock=None):
        ...
        while (ticks == 0 or ticksRun < ticks) and (self.runTime == 0 or self._time < self.runTime):
            ...
            if 'fare_alloc' in outputs:
                outputs['fare_alloc'] = self.allocated_fares
            ...
            for taxi in self._taxis.items():
                ...
                if taxi[0].onDuty:
                    ...
                    if 'return' in outputs:
                        outputs['return'][taxi[0].number] = (self._time,taxi[0].getAccount())
                        # if taxi[0].number in outputs['return']:
                        #     pass
                        # else:
                        #     outputs['return'][taxi[0].number]
            ...
            if self._dispatcher is not None:
                ...
                outputs['return']['dispatcher']=(self._time,self._dispatcher.getRevenue())
