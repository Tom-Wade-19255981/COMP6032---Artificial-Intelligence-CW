...
def runRoboUber(worldX, worldY, runTime, stop, junctions=None, streets=None, interpolate=False, outputValues=None,
                **args):
    ...
    svcArea = networld.NetWorld(x=worldX, y=worldY, runtime=runTime, fareprob=args['fareProb'], jctNodes=junctions,
                                edges=streets, interpolateNodes=interpolate, farefile=args['fareFile'])
    print("Exporting map...")
    ...
    while threadTime < threadRunTime:
        ...
        if stop.is_set():
            ...
            print("Map:\n" + str(svcArea.exportMap()))
            if threadTime != svcArea.simTime:
                threadTime += 1
            time.sleep(0.5)  # change this value to speed up the simulation. Smaller times = faster runs
    print("Thread ended")
...
pygame.init()
displaySurface = pygame.display.set_mode(size=displaySize,
                                         flags=pygame.RESIZABLE)  # |pygame.SCALED arrgh...new in pygame 2.0, but pip install installs 1.9.6 on Ubuntu 16.04 LTS
backgroundRect = None
...
activeRect = pygame.Rect(round((displaySize[0] - activeSize[0]) / 2), round((displaySize[1] - activeSize[1]) / 2),
...
for run in range(numDays):
    ...
    while curTime < runTime:
        ...
        try:
            quitevent = next(
                evt for evt in pygame.event.get() if evt.type == pygame.KEYDOWN)  # and evt.key == pygame.K_q)
            if quitevent.key == pygame.K_q:
                ...
                while not userConfirmExit.is_set():
                    try:
                        quitevent = next(evt for evt in pygame.event.get() if
                                         evt.type == pygame.KEYDOWN)  # poll for confirmation (BUGFIX 26 Nov 2024 ADR)
    ...
    print("Simulation ended, joining with robouber")
    roboUber.join()  # wait 'til the simulation thread ends (BUGFIX 26 Nov 2024 ADR)
